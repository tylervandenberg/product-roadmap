              ...
              return(
                <div key={m.id} onClick={()=>onSelectTask(m.id)}
                  ...
                  <div style={{display:"flex",gap:"8px",alignItems:"center",flexWrap:"wrap"}}>
                    ...
                    {m.date&&<span style={{fontSize:"11px",color:"#6b7280"}}>{fmtDate(m.date)}</span>}
                        ...
                        return(
                          <div key={m.id} onClick={()=>onSelectTask(m.id)}
                            ...
                            <div style={{display:"flex",gap:"5px",flexShrink:0}}>
                              {m.date&&<span style={{fontSize:"11px",color:"#374151"}}>{fmtDate(m.date)}</span>}
...
function GanttView({milestones,allMilestones,categories,chain,selectedId,hoverId,isMobile,onHover,onTap,onBgClick}){
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",overflowX:"hidden",padding:isMobile?"12px 16px":"16px 28px",position:"relative"}} onClick={e=>{e.stopPropagation();onBgClick();}}>
      {/* Month headers */}
            ...
            {items.map(m=>{
              ...
              return(
                <div key={m.id}
                  ...
                  style={{display:"flex",alignItems:"center",marginBottom:"3px",cursor:"pointer",opacity:highlightId&&!inChain?0.08:1,transition:"opacity 0.15s",minHeight:`${ROW_H}px`}}>
                  <div
                    onMouseEnter={e=>{e.stopPropagation();onHover(m.id);}}
                    onMouseLeave={e=>{e.stopPropagation();onHover(null);}}
                    style={{width:LABEL_W,flexShrink:0,fontSize:isMobile?"12px":"13px",color:isSel?color:(inChain&&highlightId)?"#e2e8f0":"#6b7280",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",paddingRight:"10px",fontWeight:isSel?"700":"400"}}>
                    {m.name}
                  </div>
                  <div
                    ...
                    {(isHov||isSel)&&(
                      <div style={{position:"absolute",left:`calc(${x}% + ${sz/2+7}px)`,top:"50%",transform:"translateY(-50%)",fontSize:"11px",color,whiteSpace:"nowrap",pointerEvents:"none",background:"#0a0a0f",padding:"2px 6px",borderRadius:"3px",border:`1px solid ${color}44`,zIndex:3}}>
                        {fmtDate(m.date)}
                      </div>
                    )}
                  </div>
  ...
  return(
    <svg style={{position:"absolute",top:0,left:0,width:"100%",height:H,pointerEvents:"none",zIndex:5}}>
      ...
      {lines.map((line,i)=>{
        ...
        // Orthogonal path: go straight up/down from x1, then horizontal to x2
...
function Detail({milestone,allMilestones,categories,settings,onToggleSetting,onClose,onNavigate}){
  ...
  const Chip=({label,col})=>(
    <span style={{display:"inline-block",padding:"3px 9px",borderRadius:"4px",background:`${col}22`,border:`1px solid ${col}55`,fontSize:"11px",color:col,marginRight:"5px",marginBottom:"5px"}}>{label}</span>
  );
            ...
            {items.map(m=>{
              ...
              return(
                <div key={m.id}
                  ...
                  onClick={e=>{e.stopPropagation();onTap(m.id);}}
                  style={{display:"flex",alignItems:"center",marginBottom:"3px",cursor:"pointer",opacity:highlightId&&!inChain?0.08:1,transition:"opacity 0.15s",minHeight:`${ROW_H}px`}}>
                  <div style={{width:LABEL_W,flexShrink:0,fontSize:isMobile?"12px":"13px",color:isSel?color:(inChain&&highlightId)?"#e2e8f0":"#6b7280",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",paddingRight:"10px",fontWeight:isSel?"700":"400"}}>
                    {m.name}
                  </div>
...
function GanttDepLines({lines,milestones,rowRefs,containerRef,LABEL_W,ROW_H,hoveredLine,setHoveredLine,selectedLine,setSelectedLine}){
  ...
  return(
    <svg style={{position:"absolute",top:0,left:0,width:"100%",height:H,pointerEvents:"none",zIndex:5}}>
      <defs>
        <marker id="gl-red" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0.5 L6,3.5 L0,6.5 Z" fill={DEP_RED}/></marker>
        <marker id="gl-green" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0.5 L6,3.5 L0,6.5 Z" fill={DEP_GREEN}/></marker>
        <filter id="gl-glow-red" x="-50%" y="-50%" width="200%" height="200%">
          <feGaussianBlur stdDeviation="2" result="b"/>
          <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
      ...
      {lines.map((line,i)=>{
        ...
        const y2=tp.top;
        const col=line.type==="blockedBy"?DEP_RED:DEP_GREEN;
        const markerId=line.type==="blockedBy"?"gl-red":"gl-green";
        const isHov=hoveredLine?.fromId===line.fromId&&hoveredLine?.toId===line.toId;
        const isSel=selectedLine?.fromId===line.fromId&&selectedLine?.toId===line.toId;
        const active=isHov||isSel;
        const mx=(x1+x2)/2;
        ...
        return(
          <g key={i}>
            ...
            <path d={d} fill="none" stroke={col}
              ...
              filter={active?`url(#gl-glow-${line.type==="blockedBy"?"red":"green"})`:undefined}
              style={{pointerEvents:"none",transition:"stroke-width 0.1s,opacity 0.1s"}}/>
            {/* Inline label on selected line */}
            {isSel&&(()=>{
              const fM=milestones.find(x=>x.id===line.fromId);
              const tM=milestones.find(x=>x.id===line.toId);
              if(!fM||!tM)return null;
              const txt=line.type==="blockedBy"?`${tM.name} ← ${fM.name}`:`${fM.name} → ${tM.name}`;
              const tw=Math.min(txt.length*6.5+16,220);
              return(
                <g transform={`translate(${lx-tw/2},${ly-11})`}>
                  <rect width={tw} height={20} rx={4} fill="#0d0d18" stroke={`${col}88`} strokeWidth={1}/>
                  <text x={tw/2} y={13} fill={col} fontSize={9} fontFamily="DM Mono,monospace" textAnchor="middle">{txt.length>30?txt.slice(0,29)+"…":txt}</text>
                </g>
              );
            })()}
          </g>
